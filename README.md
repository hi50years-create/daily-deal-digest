...
`NAVER_CLIENT_ID` / `NAVER_CLIENT_SECRET` 를 등록하면, 커머스 딜(쿠팡 골드박스 등)을
메일에 넣기 전에 **같은 상품의 네이버 쇼핑 최저가**와 비교합니다.

- 쿠팡가가 네이버 최저가보다 **15% 넘게 비싸면 → 아예 제외** ("골드박스 특가"라고
  이름만 붙고 실제론 다른 데가 싼 경우)
- 그 외에는 메일에 `✅ 이 딜이 최저가 수준` 또는 `⚠️ 네이버 최저 21,500원 — 약 30% 비쌈` 표시
- 기준(15%)은 `scripts/fetch_deals.py` 의 `MARKET_PRICE_TOLERANCE` 로 조정

네이버 개발자센터(developers.naver.com)에서 앱 등록 후 "검색" API를 추가하면 발급됩니다. 무료(하루 25,000회).
...
| `COUPANG_SECRET_KEY` | 〃 |
| `NAVER_CLIENT_ID` | developers.naver.com 앱 등록 + "검색" API 추가 후 발급 |
| `NAVER_CLIENT_SECRET` | 〃 |

- 쿠팡 키 2개가 등록되면 쿠팡 소스가 자동으로 켜집니다. 쿠팡 링크는 제휴(파트너스)
  링크라 메일 하단에 고지 문구가 자동으로 붙습니다.
- 네이버 키 2개가 등록되면 커머스 딜을 네이버 쇼핑 최저가와 대조합니다 (위 "진짜 최저가 대조" 참고).
