...
- **골드박스**: 쿠팡이 자체 지정한 그날의 특가. 관심 없는 상품(놀이공원 입장권 등)이
  섞여 있어서, 커뮤니티 딜처럼 **`LIFESTYLE_KEYWORDS` 에 걸리는 것만** 넣습니다.
- **키워드 검색**: `COUPANG_KEYWORDS` 변수(콤마구분)에 넣은 키워드로 쿠팡 상품을 검색해
  가져옵니다. **할인 여부와 무관**하게 상품 목록이라, 제휴링크·가격 참고용입니다.
  키워드마다 상위 3개. 3~6개 권장.

쿠팡 상품은 메일에 아래 형태의 블록으로 나옵니다 (API로 얻는 값만 채우고 나머지는
`(작성 필요)`). 블록을 통째로 복사해 Claude 채팅에 붙여넣으면 빈칸이 채워집니다:
...
무료로 쓸 수 있던 네이버 쇼핑 검색 API가 2026-07-31 종료되어(대체 없음),
자동 최저가 대조는 빠져 있습니다. 가격은 직접 확인하세요.
...
| `COUPANG_KEYWORDS` | (없음) | 쿠팡에서 검색할 키워드(콤마구분, 3~6개). 위 "쿠팡" 항목 참고 |
